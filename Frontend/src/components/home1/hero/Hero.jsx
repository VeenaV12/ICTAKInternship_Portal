...
const Hero = () => {
  ...
  const fetchProject = async () => {
    try {
        ...
        const response = await axios.get('https://group2vercel-backend.vercel.app/getproject', {
            headers: {
...
const downloadPDF = async (pdfLink) => {
  try {
      const response = await axios.get(`https://group2vercel-backend.vercel.app/${pdfLink}`,{
        responseType:'blob',
        credentials: 'omit',  
      })
