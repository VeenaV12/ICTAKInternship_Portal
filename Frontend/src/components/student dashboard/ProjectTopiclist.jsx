...
const ProjectTopiclist = () => {
    ...
    const handleConfirmSelection = async () => {
        try {
            const token = localStorage.getItem('token');
                const response = await axios.post('https://group2vercel-backend.vercel.app/selectproject', {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                const token = localStorage.getItem('token');
                const response = await axios.get('https://group2vercel-backend.vercel.app/displayproject', {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
