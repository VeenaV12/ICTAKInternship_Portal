...
export default function DiscussionForum() {
  ...
  useEffect(() => {
    ...
    axios.get('https://group2vercel-backend.vercel.app/getid', {
      headers: {
  ...
  const fetchPosts = () => {
    const token = localStorage.getItem('token');
    axios.get('https://group2vercel-backend.vercel.app/getpost', {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  const handleNewPostSubmit = () => {
    if (editingPostId) {
      ...
      axios.put(`https://group2vercel-backend.vercel.app/${editingPostId}`, {
      ...
      // Create new post
      axios.post(`https://group2vercel-backend.vercel.app/postquery/${loggedInStudentId}`, {
        query: newPost
      })
        .then((res) => {
  ...
  const handleNewCommentSubmit = (postId) => {
    ...
    axios.post(`https://group2vercel-backend.vercel.app/postreply/${postId}`, {
      reply: commentToSend
    }, {
      headers: {
        Authorization: `Bearer ${token}`
  ...
  const handleEditPostSubmit = (post) => {
    axios.patch(`https://group2vercel-backend.vercel.app/updatepost/${post._id}`, {
      query: editedPostContent
    })
      .then((res) => {
        console.log('Post updated successfully:', res.data);
  ...
  const handleDeletePost = () => {
    if (postToDelete) {
      axios.delete(`https://group2vercel-backend.vercel.app/deletepost/${postToDelete._id}`)
        .then((res) => {
